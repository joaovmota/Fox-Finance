  ...
  return <>
    <Modal title="Nova movimentação" onClose={() => onClose()} testId="entry-modal" titleId="entry-modal-title">
      <form className="fox-entry-form" onSubmit={submit} data-testid={`${activeType}-entry-form`}>
        ...
        <div className="fox-entry-two-columns"><Field label="Data" testId={`${activeType}-date-field`}><DatePicker value={date} onChange={setDate} testId={`${activeType}-date-input`} placeholder="Selecionar data" /></Field>{!isTransfer && <Field label="Recorrente" testId={`${activeType}-recurring-field`}><select data-testid={`${activeType}-recurring-select`}><option value="no">Não</option><option value="monthly">Mensal</option><option value="weekly">Semanal</option></select></Field>}</div>
